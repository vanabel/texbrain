...
本仓库在 [`examples/bibtex-english-chinese/`](examples/bibtex-english-chinese/README.md) 提供英文 / 中文 BibTeX 工程示例（如 `gbt7714` 与 `amsrefs`）。**在网页版 TeXbrain 中：** 欢迎页 → **Clone Repository** → **Use official TeXbrain repo (BibTeX EN/ZH example)**，或填写 `https://github.com/vanabel/texbrain.git`。克隆后在侧栏打开 `examples/bibtex-english-chinese/English-bibtex/` 或 `Chinese-bibtex/` 下的主 `.tex`，用 **Active Tab** 或 **Entry** 指向该文件后编译。经典 BibTeX 需部署端已包含 **BusyTeX**（`pnpm run download-busytex`）。说明见 [examples/bibtex-english-chinese/README.md](examples/bibtex-english-chinese/README.md)。

---

## 部署到 GitHub Pages

仓库自带工作流 `.github/workflows/deploy.yml`。若要把线上地址从默认站点换成你自己的 **fork / 自定义域名**：

1. 在 GitHub **Settings → Secrets and variables → Actions → Variables** 中设置：
