...
| **SyncTeX** | 当编译产出 `.synctex.gz`（例如 **BusyTeX XeLaTeX**，或已启用 SyncTeX 的 SwiftLaTeX pdfTeX）时：**双击编辑器区域**可将 PDF 预览定位到光标附近；在 **pdf.js** 预览页上 **Ctrl+单击**（Mac 为 **⌘+单击**）可反向跳转到对应 `.tex` 行。底部**状态栏**在入口/目标旁会显示简短提示。**说明：** `pnpm build` 后 `import.meta.env.PROD` 为真，线上预览会使用**浏览器原生 PDF 内嵌查看器**，不再走 pdf.js，因此该环境下的 **SyncTeX 精确正/反向**不可用；完整体验请在本地执行 `pnpm dev`（或非生产方式的预览）验证。 |
...
- **反向（PDF → 源码）：** 在 **pdf.js** 渲染的页面上 **Ctrl / ⌘ + 主键单击**（见上文「生产构建」限制）。
...
7. **反向代理 / CDN：** 若前面还有 nginx、Cloudflare 等，视情况清理缓存或调整静态资源缓存策略。
